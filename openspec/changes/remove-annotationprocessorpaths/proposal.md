...
当前父 POM 通过 `maven-compiler-plugin.annotationProcessorPaths` 显式管理注解处理器类路径，虽然能让编译通过，但这会让编译链更重，也偏离“模块自己声明所需依赖、Maven 默认发现处理器”的目标。
