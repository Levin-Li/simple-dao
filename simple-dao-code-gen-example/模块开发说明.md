...
## 模块基本说明
   ...
   模块采用JPA做为DAO层，模块引入 spring data-jpa。
   
   DAO参考手册：https://gitee.com/Levin-Li/simple-dao/blob/develop/manual.md

   代码生成哈希校验码：[fe431e83964cafcb1ae7e8b17b708e05], 请不要修改和删除此行内容。
...
#### 2、services 子模块
   ...
   自动生成以下类： 
       ...
       1 模块基础定义类（ModuleOption）
           代码自动生成的类，正常无需修改，定义一些模块的常量

       2 CRUD的接口类
         生成的接口类，包含增删改查的接口，以及分页查询接口。不建议修改类的内容.

       3 业务接口类,以Biz开头
         生成业务接口类，但无具体代码, 如果需要，可以添加自定义的代码。
...
#### 3、services-impl 子模块
   ...
   服务实现模块，自动生成服务实现类。
       ...
       1 CRUD的接口实现类
         生成的接口实现类，包含增删改查的接口，以及分页查询接口。不建议修改类的内容.

       3 业务接口类,以Biz开头
         生成业务接口实现类，但无具体代码, 如果需要，可以添加自定义的代码。

#### 4、starter 子模块

   自举子模块，自动生成服务实现类和以下类。
   
       1 模块插件类（ModulePlugin）
           代码自动生成的类，定义模块插件的描述，可能需要添加自定义的功能代码。
           
       2 模块自举类（ModuleStarterConfiguration） 
           代码自动生成的类，正常无需修改，主要扫描模块包名下的各种组件。

       3 模块数据初始化类（ModuleDataInitializer）
           代码自动生成的类，必要情况下，需要添加模块数据的初始化代码。
           因为基于JPA技术，一般不建议手动建表和手动倒入初始化数据。 
         
#### 4、api 子模块(包括admin-api 和 client-api)
